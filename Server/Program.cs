...
namespace Server
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            ServiceHost serviceHost = new ServiceHost(typeof(ConnectionService), new Uri("net.tcp://localhost:4444/ConnectionService"));
            serviceHost.AddServiceEndpoint(typeof(IConnection), new NetTcpBinding(), "");

            ServiceHost serviceCommunicationHost = new ServiceHost(typeof(CommunicationService), new Uri("net.tcp://localhost:4444/CommunicationService"));
            serviceCommunicationHost.AddServiceEndpoint(typeof(ICommunication), new NetTcpBinding(), "");

            ServiceHost serviceAudioHost = new ServiceHost(typeof(AudioService), new Uri("net.tcp://localhost:4444/AudioService"));
